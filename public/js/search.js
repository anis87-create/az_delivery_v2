...
const loadRestaurants = () => {
    ...
    document.getElementById('restaurants_count').innerHTML = `${restaurants.length} restaurants found`;
    searchInput.addEventListener('input', (e) => {
        restaurantContainer.innerHTML = ""; 
        let restaurantsSearched = e.target.value !== '' ? restaurants.filter(restaurant => restaurant.name.toLowerCase().includes(e.target.value.toLowerCase())) : restaurants;
        renderRestaurants(restaurantsSearched, restaurantContainer);
        document.getElementById('restaurants_count').innerHTML = `${restaurantsSearched.length} restaurants found`;
    });
    document.querySelectorAll('.category-item').forEach(cateogryItem => {
        cateogryItem.addEventListener('click',(e) => {
           restaurantContainer.innerHTML = "";  
           if(e.target.id === 'all'){
             renderRestaurants(restaurants, restaurantContainer);
...
const renderRestaurants = (arr, restaurantContainer) =>{
